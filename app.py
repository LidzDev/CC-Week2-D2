...
starting_passenger_count = bus.get_passenger_number()
print(f"There are initially {starting_passenger_count} passengers on the bus.")

bus.pick_up_passenger(person_1)
print(f"The {bus.route_number} bus picked up {bus.passengers[0].name}.")
print(f"There are now {bus.get_passenger_number()} passengers on the bus.")
...
bus.pick_up_passenger(person_1)
bus.pick_up_passenger(person_2)
bus.pick_up_passenger(person_3)
print(f"The {bus.route_number} bus picked up {bus.get_passenger_number()} passengers.")
...
# bus_stop.empty_queue()
print(f"The {bus.route_number} bus picked up {bus.get_passenger_number()} passengers from {bus_stop.name}.")
print(f"There are now {bus_stop.queue_length()}")